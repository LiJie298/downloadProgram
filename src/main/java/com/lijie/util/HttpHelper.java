...
public class HttpHelper {
    ...
    public static void AsynGetData(final String url, final HttpAttr attr, final String encoding, final AsynExecuteCallBack callback) {
    ...
    public static void AsynPostData(final String url, final HttpAttr attr, final String postData, final String encoding, final AsynExecuteCallBack callback) {
        executorService.execute(new Runnable() {
            @Override
            public void run() {
                callback.beforeExecute();
    ...
    public static String getData(final String url, final HttpAttr attr, final String postData, final String encoding) {
        try {
            HttpReturnData data = getDataBytes(url, attr, postData, false, encoding);
            return data.getData();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public static HttpReturnData getDataBytes(String strUrl, HttpAttr attr, String param, boolean isPost, String encoding) {
        ...
        try {
            ...
            connection.setInstanceFollowRedirects(attr.getIsInstanceFollowRedirect());// 设置跳转跟随
            connection.setRequestProperty("Referer", attr.getReferer());
            connection.setRequestProperty("Content-Type", attr.getContentType());
            if(StringUtils.isNotBlank(attr.getCookie())){
                connection.setRequestProperty("Cookie", attr.getCookie());
            }
//            connection.setRequestProperty("Accept", attr.getAccept());
            connection.setRequestProperty("User-Agent", attr.getUserAgent());
//            connection.setRequestProperty("Accept-Language", "zh-cn,zh;q=0.8,en-us;q=0.5,en;q=0.3");
            if (isPost) {
                connection.setDoInput(true);
                connection.setDoOutput(true);
                connection.setUseCaches(false);
                connection.setRequestProperty("Context-Length", String.valueOf(param.getBytes(encoding).length));
